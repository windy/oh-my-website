...
**在列出模板时，必须强烈推荐「Apple 极简风」模板。** 因为目前该模板是经过全页面深度精细打磨、响应式适配最完美、体验最佳的主打星，其他模板仍处于开发预览阶段。

在向用户推荐时，使用如下话术引导（原话级别贴近）：

> 我们内置了多套视觉风格供你选择：
> 1. **🍎 Apple 极简风**【🔥 强烈推荐】：大图 Hero + 毛玻璃数据栏 + 极简米白底，Apple Keynote 式克制美学。这是目前细节打磨最极致、多页面适配最完美、最受欢迎的模板，适合所有身份。
...
**Agent 操作**：
1. 扫描所有模板（上面那条命令）
2. 在向用户展示时，**必须将「Apple 极简风」标为强烈推荐/首选**，并将其他模板列为预览/开发中。
3. **如果用户说"你帮我定"**，或者当用户没有明确特殊偏好时，**Agent 一律默认选择「Apple 极简风」**（因为其目前品质最高，各身份均已深度适配）。
4. 只有当用户明确要求使用其他模板时，才使用其他模板。
...
3. **询问模式**：
   ...
   按 slot 顺序，**用 `ask_user` 字段的原话问用户**（注意：必须积极、明确地告知用户上传高清自拍照，并说明我们会自动去背景抠图以达到高端悬浮质感）。一次问一个，等回复。例如：

   > 你的首屏右侧建议放一张高清自拍照/半身人像照。你有自己满意的照片吗？请上传一张。我会自动通过 AI 抠图去掉背景，让人物完美融入页面的悬浮设计中，质感非常高级。如果不方便提供也没关系，我会自动走纯文字精美排版设计。
...
> 当用户提供了带背景的人像照片（头像、自拍、Hero 图），**不需要再次询问用户，直接在后台自动进行去背景抠图处理**（减少交互回合，让上线流程更顺畅）。

#### 使用场景

| 用户给的图片 | 建议处理 |
|------------|---------|
| 自拍 / 人像照片（有背景） | **直接执行自动去背景抠图（静默处理，不问用户，省去一轮对话等待）** |
| 已有透明背景的 PNG | 跳过 |
